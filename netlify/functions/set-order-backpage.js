// netlify/functions/set-order-backpage.js
//
// Admin-only. Захиалгад ар хавтасны зургийг хадгална (эсвэл шинэчилнэ).
//
// Хүлээн авах (POST JSON): { id, imageBase64, backpageDescription }
// Header: x-admin-pin
...
exports.handler = async (event) => {
  ...
  const { id, imageBase64, backpageDescription } = body;
  ...
  try {
    ...
    const backpageImageKey = `${id}:backpage`;
    await saveOrderImage(backpageImageKey, imageBase64);

    order.backpageImageKey = backpageImageKey;
    order.backpageDescription = backpageDescription || order.backpageDescription || "";
    ...
    console.error("set-order-backpage error:", err);
